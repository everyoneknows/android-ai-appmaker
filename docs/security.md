...
APIキー・パスワード・トークンをログへ出さず、リポジトリへ保存しません。署名鍵は端末内にのみ置きます。curl|bashはREADMEのimmutable commit SHAをsetupの環境変数にも渡し、Google archiveはSHA-256と内部pathを検証します。実行前にsetup.shを確認してください。
