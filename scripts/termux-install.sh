...
required=(curl unzip zip python openjdk-21 coreutils); pkg update -y
for package in "${required[@]}"; do
  pkg install -y "$package" >/dev/null || { echo "必須パッケージを導入できませんでした: $package" >&2; exit 1; }
  case "$package" in curl) command -v curl;; unzip) command -v unzip;; zip) command -v zip;; python) command -v python;; openjdk-21) command -v javac && command -v java && command -v keytool;; coreutils) command -v realpath && command -v readlink && command -v sha256sum && command -v find && command -v xargs;; esac >/dev/null || { echo "コマンドが見つかりません: $package" >&2; exit 1; }
done
commands=(curl unzip zip python javac java keytool realpath readlink sha256sum find xargs)
