...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  if self.path not in {'/build','/install'}: self.send_error(404); return
  ...
  if self.path=='/install':
   if n != 0: self.send_json(400,{'message':'installリクエストの本文は空である必要があります'}); return
   apk=Path.home()/'android-ai-appmaker/out/latest/app.apk'
   if not apk.is_file(): self.send_json(404,{'message':'APKがまだありません'}); return
   try:
    subprocess.Popen(['termux-open',str(apk)]); self.send_json(200,{'message':'Androidのインストール画面を開きました。'})
   except OSError:
    logger.exception('install launch failed')
    self.send_json(500,{'message':'APKを開けません。Termuxのログを確認してください。'})
   return
